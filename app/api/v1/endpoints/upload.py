...
async def upload_document(
    ...
    # 문서 처리
    try:
        result = await doc_service.process_and_store_document(file)
        return result
...
async def search_documents(
    ...
    try:
        results = doc_service.search_documents(query=query, top_k=top_k)
...
async def get_document_info(
    ...
    """문서 정보 조회"""
    try:
        info = doc_service.get_document_info(document_id)
        return info
    except ValueError as e:
